		...
		if (OGMO.ctrl)
		{
			...
			else if (key == Keys.V && DecalSelectTool.inClipboard.length > 0)
			{
				...
				for (decal in DecalSelectTool.inClipboard)
				{
					...
					for (value in decal.values) {
						newValues.push(value.clone());
					}
					var clone = new Decal(null, decal.position.clone(), decal.path, decal.texture, decal.origin.clone(), decal.scale.clone(), decal.rotation, newValues);
			...
			else if (key == Keys.D && layerEditor.selected.length > 0)
			{
				...
				for (decal in layerEditor.selected)
				{
					...
					var clone = new Decal(null, decal.position.clone().add(new Vector(32, 32)), decal.path, decal.texture, decal.origin.clone(), decal.scale.clone(), decal.rotation, newValues);
					(cast layerEditor.layer:DecalLayer).decals.push(clone);
